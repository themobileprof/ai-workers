...
// Store is Postgres database aiworkers (never the n8n database). Schema is created in migrate.
type Store struct {
	pool *pgxpool.Pool
}
...
`)
	...
	if err := s.migrateDesks(ctx); err != nil {
		return err
	}
	return s.migrateCommunity(ctx)
}
