...
4. **`community` (Community Manager)**
    - *Welcome, weekly intro, progress, lesson help, FAQ, moderation, engagement.* One **mandate** per WhatsApp group (desk `/admin/community`): brief + catalog + group id. n8n attaches it via `GET /internal/v1/community/mandate`. Monday 09:00 Lagos (`commWeeklyPing01`) introduces the host and asks what people are working on. Stay silent on chatter (`speak: false`). Escalate to a human when the catalog does not cover it. Not sales and not ops. On WhatsApp, unauthorized `/accounts`, `/ops`, `/legal`, or `/hr` is `access_denied` — those departments never run.
