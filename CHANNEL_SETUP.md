...
- **People** (`/admin/users`, owner only): add a person, office seat (owner / bdm / cofounder / assistant / viewer), and **one or more handler desks**. Each desk is `/admin/desks/{slug}` — Ask worker, chat the thread, Accept / Turn away. Chat does not stamp. Owner sees every desk. Ticking a desk grants `web_admin` so the handler can sign in. Specialized books stay at `/admin/legal`, `/admin/hr`, `/admin/projects`.
